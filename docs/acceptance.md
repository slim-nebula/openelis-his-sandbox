...
| Access control | Four controls for four kinds of caller: the estate's user token (HS256 + Redis revocation) on the clinical API; `x-internal-api-key` between services; a shared operator token, fail-closed and fixed-time compared, on everything that changes something or exposes operational detail; origin restriction on `/fhir`, because OpenELIS cannot present a credential. All fail-closed when unset. `docs/security.md` §§3–6; asserted on status codes in `make negative` and `make auth` |
