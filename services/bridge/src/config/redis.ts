 ...
 */
export const redis: Redis | null = config.redis.host
  ? new Redis({
...
if (redis) {
  ...
  redis.on('ready', () => {
    if (!redisHealth.available) logger.info('Redis reachable — token revocation is being enforced');
    redisHealth.available = true;
  });
  redis.on('error', (error: Error) => {
    if (redisHealth.available) {
      logger.warn(
        `Redis unreachable (${error.message}) — continuing in degraded authentication mode: ` +
  ...
  logger.warn(
    'No Redis address configured: estate tokens will be accepted on signature alone, ' +
