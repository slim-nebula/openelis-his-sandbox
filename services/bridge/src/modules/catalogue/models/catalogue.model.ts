...
import type { ICatalogueDiff, ICatalogueEntry, ISyncHistoryRow } from '../types/catalogue.types.js';

const toEntry = (row: Row): ICatalogueEntry => ({
  ...
  specimenAbbreviation:
    row.specimen_abbrev === null || row.specimen_abbrev === undefined
      ? null
...
export class CatalogueModel {
  ...
  async all(): Promise<ICatalogueEntry[]> {
    const rows = await query<Row>(
      `SELECT loinc, openelis_test_id, name, specimen_name, specimen_id,
  ...
  async syncedAt(): Promise<string | null> {
    const row = await queryOne<Row>('SELECT max(synced_at) AS synced_at FROM bridge.test_catalogue');
   ...
   */
  async replaceAll(entries: ICatalogueEntry[]): Promise<void> {
  ...
  async beginSync(testsBefore: number): Promise<number> {
  ...
  async syncHistory(): Promise<ISyncHistoryRow[]> {
    const rows = await query<Row>(
      `SELECT id, started_at, finished_at, status, tests_before, tests_after,
