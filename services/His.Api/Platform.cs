...
///
/// This service stands in for patient-service, so it has to carry the same
/// hooks its real counterpart does: register with Consul, answer /health in the
/// agreed shape, expose /metrics, and put its logs on the estate's shared topic.
///
/// It matters here for a different reason than it does in the bridge. The
/// bridge needs these because it will be deployed. This one needs them because
/// it will be READ — a reference implementation that skips the platform wiring
/// teaches the wrong lesson twice: once to whoever copies it, and again to
/// whoever later wonders why the real service has code the example did not.
///
/// Kept deliberately identical to the bridge's copy. The two services share no
/// project — each Dockerfile builds from its own directory — so this is
/// duplicated rather than referenced. When one changes, change both.
/// </summary>
    ...
    bool ConsulEnabled)
{
    public static PlatformOptions FromEnvironment(string defaultServiceName) => new(
        ServiceName: KafkaOptions.Env("SERVICE_NAME", defaultServiceName),
        ServiceVersion: KafkaOptions.Env("SERVICE_VERSION", "1.0.0"),
        ConsulHost: KafkaOptions.Env("CONSUL_HOST", ""),
        ConsulPort: int.Parse(KafkaOptions.Env("CONSUL_PORT", "8500")),
        ...
        AdvertisedIp: KafkaOptions.Env("SERVICE_IP", "") is { Length: > 0 } ip ? ip : null,
        Port: int.Parse(KafkaOptions.Env("SERVICE_PORT", "8080")),
        LogsTopic: KafkaOptions.Env("TOPIC_LOGS", "logs"),
        ConsulEnabled: KafkaOptions.Env("CONSUL_HOST", "").Length > 0);
}
